...
gen_sim_study_brk <- function(data_type = "lin", n_ic_mod = 500, n_ic_h = 500, n_oc = 500,
  ...
  # Fit Models
  fit_gru <- train(dat$none[id_trn_mod, ], method = "gruMCUSUM", lags = l, k = 1.1)
  fit_mrf <- train(dat$none[id_trn_mod, ], method = "mrfMCUSUM", lags = l, k = 5)
  fit_vmc <- train(dat$none[id_trn_mod, ], method = "varmaMCUSUM", lags = 1, k = 0.9)
  fit_vmw <- train(dat$none[id_trn_mod, ], method = "varmaMEWMA", lags = 1, r = 0.3)
  fit_hts <- train(dat$none[id_trn_mod, ], method = "htsquare")
  
  # Predict: To estimate h
  pred_gru_h <- predict(fit_gru, dat$none[id_trn_h, ])
  pred_mrf_h <- predict(fit_mrf, dat$none[id_trn_h, ])
  pred_vmc_h <- predict(fit_vmc, dat$none[id_trn_h, ])
  pred_vmw_h <- predict(fit_vmw, dat$none[id_trn_h, ])
  pred_hts_h <- predict(fit_hts, dat$none[id_trn_h, ])
  
  # Get h Level
  h_gru <- quantile(pred_gru_h$pstat, ql)
  h_mrf <- quantile(pred_mrf_h$pstat, ql)
  h_vmc <- quantile(pred_vmc_h$pstat, ql)
  h_vmw <- quantile(pred_vmw_h$pstat, ql)
  h_hts <- quantile(pred_hts_h$pstat, ql)
  ...
  list(dat_pstat = bind_rows(pstat_gru,
       ...
       dat_rl = bind_rows(rl_gru,
                          ...
                          rl_hts) |> 
         mutate(Data = as_factor(Data), method = as_factor(method)),
       dat_h = tibble(h = c("gruMCUSUM", "mrfMCUSUM", "varMCUSUM",
                            "varMEWMA", "Hotelling-T2"),
                      value = c(h_gru, h_mrf, h_vmc, h_vmw, h_hts)))
}
