...
ggsave(paste0(fig_path, "example-data-exog.png"),
       width = 25, height = 20, units = "cm")

# Time series plots cell 1 and perm cond. 1
dat |> 
  pivot_longer(c("Feed Cond. (mS/cm)", "Feed Pump % Power")) |> 
  ggplot(aes(Date_Time, value)) +
  geom_line() + #geom_point(shape = 1) +
  geom_vline(xintercept = end_trn, color = "blue") +
  geom_vline(xintercept = end_tst, color = "red") +
...
ggsave(paste0(fig_path, "example-data-exog-1.png"),
       width = 25, height = 10, units = "cm")


# Combine model residuals for all data
fit      <- readRDS(here("data", "fit-exog.rds"))
pred_ctl <- readRDS(here("data", "pred-ctl-exog.rds"))
pred_tst <- readRDS(here("data", "pred-tst-exog.rds"))
...
res <- names(pred_tst) |> 
  ...
  set_names(c("GRU", "MRF", "Centered Data"))
...
1:length(res) |> 
  walk(\(i) {
    ggsave(filename = paste0(fig_path, "res-exog-", names(res)[i], ".png"),
           plot = res_plots[[i]], width = 30, height = 10, units = "cm")
  })

# Read plotting statistics and control limit
pstat <- readRDS(here("data", "pstat-exog.rds"))
h     <- readRDS(here("data", "h-exog.rds"))

# y limits for each graph
y_lims <-
  map2(pstat, c(.99, .99, .99), \(x, q) {
    quantile(x$value, q)
  })
