### Imports ###
library("here")
library("tidyverse")
theme_set(theme_bw())
library("reticulate")
library("mlmcusum")
library("furrr")
library("kableExtra")

use_condaenv("/home/nossimid/miniconda3/envs/deep_learning_v03")
path_py <- "~/git/mlmcusum/inst/python/gru_functions.py"
source_python(path_py)

#set.seed(123)

# Parameters
# Method: GRU, data linear (first past)
sim_h <- function(n_ic_trn = 500, n_ic_tst = 500, l = 4, arl = 200) {
  # Constants
  id_trn <- 1:n_ic_trn
  id_tst <- (n_ic_trn - l):(n_ic_trn+n_ic_tst) # adjust l for varma and htsquare
  ql <- 1 - 1/arl
  
  ### Linear Data ###
  dat_lin <- gen_dat_lin(n_ic_trn + n_ic_tst, n_oc = 0)
  
  # Fit Models
  fit_gru_lin <- train(dat_lin$none[id_trn, ], method = "gruMCUSUM", lags = l, k = 1.1)
  fit_mrf_lin <- train(dat_lin$none[id_trn, ], method = "mrfMCUSUM", lags = l, k = 5)
  fit_vmc_lin <- train(dat_lin$none[id_trn, ], method = "varmaMCUSUM", lags = 1, k = 0.9)
  fit_vmw_lin <- train(dat_lin$none[id_trn, ], method = "varmaMEWMA", lags = 1, r = 0.3)
  fit_hts_lin <- train(dat_lin$none[id_trn, ], method = "htsquare")
  
  # Predict: GRU
  pred_gru_lin_nf <- predict(fit_gru_lin, dat_lin$none[id_tst, ])
  
  # Predict: MRF
  pred_mrf_lin_nf <- predict(fit_mrf_lin, dat_lin$none[id_tst, ])
  
  # Predict: VARMA - MCUSUM
  pred_vmc_lin_nf <- predict(fit_vmc_lin, dat_lin$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: VARMA - MEWMA
  pred_vmw_lin_nf <- predict(fit_vmw_lin, dat_lin$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: Hotelling's T^2
  pred_hts_lin_nf <- predict(fit_hts_lin, dat_lin$none[id_tst[-c(1:(l-1))], ])
  
  # Get h Level
  h_gru_lin <- quantile(pred_gru_lin_nf$pstat[(l+1):n_ic_tst], ql)
  h_mrf_lin <- quantile(pred_mrf_lin_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmc_lin <- quantile(pred_vmc_lin_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmw_lin <- quantile(pred_vmw_lin_nf$pstat[(l+1):n_ic_tst], ql)
  h_hts_lin <- quantile(pred_hts_lin_nf$pstat[(l+1):n_ic_tst], ql)
  
  # Construct Tibble
  rl_gru_lin <- tibble(Data = "linear", method = "GRU-MCUSUM", h = h_gru_lin)
  rl_mrf_lin <- tibble(Data = "linear", method = "MRF-MCUSUM", h = h_mrf_lin)
  rl_vmc_lin <- tibble(Data = "linear", method = "VMC-MCUSUM", h = h_vmc_lin)
  rl_vmw_lin <- tibble(Data = "linear", method = "VMW-MEWMA",  h = h_vmw_lin)
  rl_hts_lin <- tibble(Data = "linear", method = "Hotelling-T2", h = h_hts_lin)
  
  ### Non-Linear Data ###
  dat_nlr <- gen_dat_nlr(n_ic_trn + n_ic_tst, n_oc = 0)
  
  # Fit Models
  fit_gru_nlr <- train(dat_nlr$none[id_trn, ], method = "gruMCUSUM", lags = l, k = 1.1)
  fit_mrf_nlr <- train(dat_nlr$none[id_trn, ], method = "mrfMCUSUM", lags = l, k = 5)
  fit_vmc_nlr <- train(dat_nlr$none[id_trn, ], method = "varmaMCUSUM", lags = 1, k = 0.9)
  fit_vmw_nlr <- train(dat_nlr$none[id_trn, ], method = "varmaMEWMA", lags = 1, r = 0.3)
  fit_hts_nlr <- train(dat_nlr$none[id_trn, ], method = "htsquare")
  
  # Predict: GRU
  pred_gru_nlr_nf <- predict(fit_gru_nlr, dat_nlr$none[id_tst, ])
  
  # Predict: MRF
  pred_mrf_nlr_nf <- predict(fit_mrf_nlr, dat_nlr$none[id_tst, ])
  
  # Predict: VARMA - MCUSUM
  pred_vmc_nlr_nf <- predict(fit_vmc_nlr, dat_nlr$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: VARMA - MEWMA
  pred_vmw_nlr_nf <- predict(fit_vmw_nlr, dat_nlr$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: Hotelling's T^2
  pred_hts_nlr_nf <- predict(fit_hts_nlr, dat_nlr$none[id_tst[-c(1:(l-1))], ])
  
  # Get h Level
  h_gru_nlr <- quantile(pred_gru_nlr_nf$pstat[(l+1):n_ic_tst], ql)
  h_mrf_nlr <- quantile(pred_mrf_nlr_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmc_nlr <- quantile(pred_vmc_nlr_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmw_nlr <- quantile(pred_vmw_nlr_nf$pstat[(l+1):n_ic_tst], ql)
  h_hts_nlr <- quantile(pred_hts_nlr_nf$pstat[(l+1):n_ic_tst], ql)
  
  # Construct Tibble
  rl_gru_nlr <- tibble(Data = "non-linear", method = "GRU-MCUSUM", h = h_gru_nlr)
  rl_mrf_nlr <- tibble(Data = "non-linear", method = "MRF-MCUSUM", h = h_mrf_nlr)
  rl_vmc_nlr <- tibble(Data = "non-linear", method = "VMC-MCUSUM", h = h_vmc_nlr)
  rl_vmw_nlr <- tibble(Data = "non-linear", method = "VMW-MEWMA",  h = h_vmw_nlr)
  rl_hts_nlr <- tibble(Data = "non-linear", method = "Hotelling-T2", h = h_hts_nlr)
  
  ### Long-Term Data ###
  dat_ltm <- gen_dat_ltm(n_ic_trn + n_ic_tst, n_oc = 0)
  
  # Fit Models
  fit_gru_ltm <- train(dat_ltm$none[id_trn, ], method = "gruMCUSUM", lags = l, k = 1.1)
  fit_mrf_ltm <- train(dat_ltm$none[id_trn, ], method = "mrfMCUSUM", lags = l, k = 5)
  fit_vmc_ltm <- train(dat_ltm$none[id_trn, ], method = "varmaMCUSUM", lags = 1, k = 0.9)
  fit_vmw_ltm <- train(dat_ltm$none[id_trn, ], method = "varmaMEWMA", lags = 1, r = 0.3)
  fit_hts_ltm <- train(dat_ltm$none[id_trn, ], method = "htsquare")
  
  # Predict: GRU
  pred_gru_ltm_nf <- predict(fit_gru_ltm, dat_ltm$none[id_tst, ])
  
  # Predict: MRF
  pred_mrf_ltm_nf <- predict(fit_mrf_ltm, dat_ltm$none[id_tst, ])
  
  # Predict: VARMA - MCUSUM
  pred_vmc_ltm_nf <- predict(fit_vmc_ltm, dat_ltm$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: VARMA - MEWMA
  pred_vmw_ltm_nf <- predict(fit_vmw_ltm, dat_ltm$none[id_tst[-c(1:(l-1))], ])
  
  # Predict: Hotelling's T^2
  pred_hts_ltm_nf <- predict(fit_hts_ltm, dat_ltm$none[id_tst[-c(1:(l-1))], ])
  
  # Get h Level
  h_gru_ltm <- quantile(pred_gru_ltm_nf$pstat[(l+1):n_ic_tst], ql)
  h_mrf_ltm <- quantile(pred_mrf_ltm_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmc_ltm <- quantile(pred_vmc_ltm_nf$pstat[(l+1):n_ic_tst], ql)
  h_vmw_ltm <- quantile(pred_vmw_ltm_nf$pstat[(l+1):n_ic_tst], ql)
  h_hts_ltm <- quantile(pred_hts_ltm_nf$pstat[(l+1):n_ic_tst], ql)
  
  # Construct Tibble
  rl_gru_ltm <- tibble(Data = "long-term", method = "GRU-MCUSUM", h = h_gru_ltm)
  rl_mrf_ltm <- tibble(Data = "long-term", method = "MRF-MCUSUM", h = h_mrf_ltm)
  rl_vmc_ltm <- tibble(Data = "long-term", method = "VMC-MCUSUM", h = h_vmc_ltm)
  rl_vmw_ltm <- tibble(Data = "long-term", method = "VMW-MEWMA",  h = h_vmw_ltm)
  rl_hts_ltm <- tibble(Data = "long-term", method = "Hotelling-T2", h = h_hts_ltm)
  
  
  ### Return
  list(h_vals = bind_rows(rl_gru_lin,
                          rl_mrf_lin,
                          rl_vmc_lin,
                          rl_vmw_lin,
                          rl_hts_lin,
                          rl_gru_nlr,
                          rl_mrf_nlr,
                          rl_vmc_nlr,
                          rl_vmw_nlr,
                          rl_hts_nlr,
                          rl_gru_ltm,
                          rl_mrf_ltm,
                          rl_vmc_ltm,
                          rl_vmw_ltm,
                          rl_hts_ltm) |> 
         mutate(Data = as_factor(Data), method = as_factor(method)))
}

### Run Single Simulation
sim_tst <- sim_h(n_ic_trn = 400, n_ic_tst = 400)

### Multiple Simulations
future::plan(multisession)
n_sim <- 100

sim_vals <- 1:n_sim |> 
  future_map(\(i) sim_h(n_ic_trn = 1000, n_ic_tst = 2000))

future::plan(sequential)

# h Value Table
dat_h <- sim_vals |>
  map(\(i) i$h_vals) |> 
  bind_rows() |> 
  group_by(Data, method) |> 
  summarise(across(where(is.numeric), mean))

# Save Results
save(dat_h, file = here("results", "h.rda"))

dat_h |> 
  mutate(h = round(h, 1)) |> 
  kbl(format = "latex", booktabs = TRUE)
